...
def generate_festival_data_js(static_dst):
    ...
    Hebrew dates (Passover, Hanukkah) via pyluach — accurate for any year.
    Islamic dates (Ramadan, Eid al-Fitr, Eid al-Adha) via tabular algorithm;
    accurate to ±1–2 days vs. regional moon sighting.

    Covers Gregorian years 1990–2150. Replaces the hardcoded tables that
    were previously in year-festivals.js and year-page.js.
    """
    import math
    ...
    for g_year in range(YEAR_START, YEAR_END + 1):
    ...
    print(f"  {len(passover_rows)} Passovers, {len(hanukkah_rows)} Hanukkah,"
          f" {len(islamic_rows)} Islamic years ({YEAR_START}–{YEAR_END})")
