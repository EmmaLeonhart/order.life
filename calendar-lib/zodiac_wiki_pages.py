...
# BC dates CSV files (generated by C# program)
BC_DATES_CSV = os.path.join('GaianDateRangeGenerator', 'gaian_minimal.csv')  # Use the correct CSV
BC_DETAILED_CSV = "detailed_bc_dates.csv"     # Day-by-day mappings

# Global cache for BC dates data
_bc_dates_cache = None

def load_bc_dates_csv(csv_file: str = BC_DATES_CSV) -> Dict[int, Dict]:
    """Load BC dates CSV data and return as a dictionary keyed by Gaian year."""
    global _bc_dates_cache
    
    if _bc_dates_cache is not None:
        return _bc_dates_cache
    
    _bc_dates_cache = {}
    csv_path = Path(csv_file)
    
    if not csv_path.exists():
        print(f"Warning: BC dates CSV file '{csv_file}' not found. BC years will be unavailable.")
        return _bc_dates_cache
    ...
    try:
        with open(csv_path, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                gaian_year = int(row['GaianYear'])
                
                # Parse start date from format "December 29, 9993 BC"
                start_date_str = row['StartDate']
                start_date = parse_bc_date_string_minimal_format(start_date_str)
                
                # Calculate ISO year from Gaian year
                iso_year = gaian_year - 10000
                
                # Get other info from CSV
                days_in_year = int(row['DaysInYear'])
                has_intercalary = row['GaianLeapYear'].lower() == 'true'
                
                # For end date, calculate based on days in year (approximate)
                # This is just for display purposes since the exact end date isn't critical
                from datetime import datetime, timedelta
                if start_date and isinstance(start_date, dict):
                    # Create a rough end date estimate
                    end_date = {
                        'is_bc': True,
                        'bc_year': start_date['bc_year'],
                        'month': 12,
                        'day': 31,
                        'original_str': f"{start_date['bc_year']:04d}-12-31 BC"
                    }
                else:
                    end_date = None
                
                _bc_dates_cache[gaian_year] = {
                    'start_date': start_date,
                    'end_date': end_date,
                    'iso_year': iso_year,
                    'days_in_year': days_in_year,
                    'has_intercalary': has_intercalary,
                    'start_weekday': 'Monday',  # Default, can be calculated if needed
                    'end_weekday': 'Sunday'     # Default, can be calculated if needed
                }
                
    except Exception as e:
        print(f"Error loading BC dates CSV: {e}")
        _bc_dates_cache = {}
    
    return _bc_dates_cache

def parse_bc_date_string_minimal_format(date_str: str) -> dict:
    """Parse a BC date string like 'December 29, 9993 BC' into a BC date dict."""
    try:
        # Parse "December 29, 9993 BC" format
        parts = date_str.split(', ')
        if len(parts) >= 2:
            month_day = parts[0]  # "December 29"
            year_part = parts[1]  # "9993 BC"
            
            month_name, day_str = month_day.split(' ')
            day_num = int(day_str)
            bc_year = int(year_part.replace(' BC', ''))
            ...
            return {
                ...
                'month': month_num,
                'day': day_num,
                'original_str': f"{bc_year:04d}-{month_num:02d}-{day_num:02d} BC"
            }
...
def parse_bc_date_string(date_str: str) -> date:
    """Parse a BC date string like '0011-01-01 BC' into a special BC date object."""
    if ' BC' in date_str:
        # BC date - store the original string and parse info for later use
        date_part = date_str.replace(' BC', '')
        year, month, day = map(int, date_part.split('-'))
        
        # For BC dates, we'll store them as a special dict with the BC info
        # since Python date() can't handle BC years directly
        return {
            'is_bc': True,
            'bc_year': year,
            'month': month,
        ...
        # AD date - return normal date object
...
def get_bc_year_info(gaian_year: int) -> Dict:
    ...
    bc_data = load_bc_dates_csv()
    return bc_data.get(gaian_year, {})

def format_bc_date_for_display(date_obj) -> str:
    """Format a BC date object for display."""
    if isinstance(date_obj, dict) and date_obj.get('is_bc'):
...
def load_bc_detailed_data_for_year(year: int) -> List[Dict]:
    """Load detailed day-by-day mappings for a specific BC year."""
    detailed_csv = Path(BC_DETAILED_CSV)
        ...
        with open(detailed_csv, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                if int(row['GaianYear']) == year:
                    year_data.append({
                        'month': int(row['GaianMonth']),
                        'day': int(row['GaianDay']),
                        'month_name': row['MonthName'],
                        'gregorian_date': row['GregorianDate'],
                        'weekday': row['Weekday']
                    })
    except Exception as e:
        print(f"Error loading detailed BC data for year {year}: {e}")
        return []
...
def extract_overview_section(page_content: str) -> str:
    """Extract the Overview section from existing page content, including all subheadings."""
    if not page_content:
        return ""
    ...
    for line in lines:
        if line.strip() == "== Overview ==":
            in_overview = True
            continue
        elif in_overview and line.startswith("== ") and not line.startswith("=== "):
            # Found another level-2 section (== Section ==), stop extracting
            # But continue if it's a subsection (=== Subsection ===, ==== etc)
            break
        elif in_overview:
...
def get_month_date_ranges(m_idx: int, span: int = 10) -> str:
    ...
    for gaian_year in sorted(year_ranges.keys()):
        ...
        start_str = format_bc_date_for_display(start_date)
        end_str = format_bc_date_for_display(end_date)
        lines.append(f"|-\n| {gaian_year} || {start_str} || {end_str} || {greg_years_str}")
...
def get_year_date_range(year: int) -> tuple:
    ...
    # For BC years, use CSV data
    if is_bc_year(year):
        bc_info = get_bc_year_info(year)
        if bc_info:
            return bc_info['start_date'], bc_info['end_date']
        else:
            # BC year not in CSV data
            return None, None
...
def get_gregorian_year_overlaps(year: int) -> list:
    ...
    # Handle BC dates (stored as dicts) vs regular dates
    def get_year_from_date_obj(date_obj):
        if isinstance(date_obj, dict) and date_obj.get('is_bc'):
            return -date_obj['bc_year']  # Convert to negative for BC
        else:
            return date_obj.year
    
    start_year = get_year_from_date_obj(start_date)
    end_year = get_year_from_date_obj(end_date)
...
def get_year_start_weekday_category(year: int) -> str:
    """Get the category for when a Gaian year starts based on Gregorian date (leap/common year beginning on Dec 29 etc.)."""
    from datetime import date
    import calendar
    import csv
    import os
    
    # Determine if this is a Gaian leap/common year and get start date info
    iso_year = year - 10000
    ...
    if iso_year <= 0:  # BC year
        ...
        if os.path.exists(csv_path):
            with open(csv_path, 'r') as f:
                ...
                for row in reader:
                    if int(row['GaianYear']) == year:
                        # Get the start date string like "January 3, 9877 BC"
                        start_date_str = row['StartDate']
                        has_intercalary = row.get('GaianLeapYear', 'False').lower() == 'true'
                        is_gregorian_leap = row.get('GregorianLeapYear', 'False').lower() == 'true'
                        
                        try:
                            # Parse "January 3, 9877 BC" format
                            parts = start_date_str.split(', ')
                            if len(parts) >= 2:
                                ...
                                date_str = f"{month_abbrev_map[month_name]} {day_num}"
                                greg_leap_str = "greg leap" if is_gregorian_leap else "greg common"
                                
                                # Format the category
                                year_type = "Leap years" if has_intercalary else "Common years"
                                return f"[[Category:{year_type} beginning on {date_str} ({greg_leap_str})]]"
                                
                        except (ValueError, KeyError):
                            pass
                        break
        
        # If not found in CSV, try to extract from calendar table
        table = build_gregorian_correspondence_table(year)
        if table:
            lines = table.split('\n')
            
            # Look for Sagittarius 1 date in the table
            for i, line in enumerate(lines):
                if 'Sagittarius 1|1]]' in line:
                    # Look for the date line (skip |- separator)
                    for j in range(i+1, min(i+4, len(lines))):  # Look in next 3 lines
                        date_line = lines[j]
                        if '[[:en:' in date_line:  # This is the date line
                            # Extract the first date from the date line
                            import re
                            date_matches = re.findall(r'\[\[:en:([A-Za-z]+ \d+)\|', date_line)
                            if date_matches:
                                first_date = date_matches[0]  # Like "December 30"
                                
                                # Parse the date
                                try:
                                    month_name, day_str = first_date.split(' ')
                                    day_num = int(day_str)
                                    ...
                                    # For years not in CSV, make reasonable assumptions
                                    # Most early BC years are common and the Gregorian year they correspond to
                                    bc_year = abs(iso_year - 1)
                                    ...
                                    # Assume common years for years not in CSV (they're probably early years)
                                    # You could improve this by checking if the table has Horus
                                    has_horus = 'Horus' in table
                                    year_type = "Leap years" if has_horus else "Common years"
        ...
        return ""  # Can't determine date for this BC year
            
    else:  # AD year
        # Get start date using ISO calendar
        try:
            start_date = date.fromisocalendar(iso_year, 1, 1)
        except ValueError:
            return ""  # Can't calculate start date for this year
        
        # Check if Gaian leap year
        try:
            date.fromisocalendar(iso_year, 53, 1)
        ...
        date_str = f"{month_names[start_date.month]} {start_date.day}"
        
        # Check if the Gregorian year is leap
        is_gregorian_leap = False
        if iso_year % 4 == 0:
            if iso_year % 100 == 0:
                if iso_year % 400 == 0:
...
def build_bc_holiday_dates_for_year(year: int) -> str:
    """Build holiday dates table for BC years (fixed-date and weekday holidays only)."""
    bc_info = get_bc_year_info(year)
    if not bc_info:
        return f"<!-- Holiday dates not available for BC year {year} GE (not in BC dates CSV) -->"
    
    detailed_data = load_bc_detailed_data_for_year(year)
    if not detailed_data:
        return f"<!-- Holiday dates require detailed BC data for year {year} GE -->"
    ...
    # Create a lookup map for Gregorian date to Gaian date conversion
    greg_to_gaian = {}
    for entry in detailed_data:
        # Parse Gregorian date from format "YYYY-MM-DD BC"
        greg_str = entry['gregorian_date']
        if ' BC' in greg_str:
            date_part = greg_str.replace(' BC', '')
            try:
                # Extract month and day for lookup
                parts = date_part.split('-')
                if len(parts) == 3:
                    month = int(parts[1])
                    day = int(parts[2])
                    greg_to_gaian[(month, day)] = {
    ...
    # 1. Fixed date holidays
    for name, (month, day) in FIXED_DATE_EVENTS.items():
        if (month, day) in greg_to_gaian:
            gaian_info = greg_to_gaian[(month, day)]
            month_name = gaian_info['month_name']
            gaian_day = gaian_info['day']
            greg_display = f"{MONTH_NAMES[month-1]} {day}"
            holidays.append((name, f"[[{month_name} {gaian_day}]]", greg_display, "Fixed"))
    
    # 2. Weekday holidays are not calculated for BC years due to complexity
    # (Would need to implement weekday calculation for BC dates)
    
    if not holidays:
        return f"== Holiday dates in {year_label} ==\n" \
               f"<small>No fixed-date holidays found for this year. " \
               f"Weekday-based holidays are not calculated for BC years.</small>"
    
    # Build the table
    lines = [f"== Holiday dates in {year_label} =="]
    lines.append('{| class="wikitable sortable"')
    lines.append('! Holiday !! Gaian Date !! Gregorian Date !! Type')
    
    for name, gaian_date, gregorian_date, holiday_type in holidays:
        lines.append(f"|-\n| {name} || {gaian_date} || {gregorian_date} || {holiday_type}")
    
    lines.append('|}')
    lines.append(f"<small>Fixed-date holidays for Gaian year {year} GE ({year_label}). " \
                 f"Weekday-based holidays are not calculated for BC years.</small>")
...
def build_year_page(year: int, wiki: 'Wiki' = None) -> (str, str):
    ...
    if start_date and end_date:
        start_str = format_bc_date_for_display(start_date)
        end_str = format_bc_date_for_display(end_date)
        parts.append(f"\n{year} goes from {start_str} to {end_str}.")
    ...
    # Add weekday-based category
    weekday_category = get_year_start_weekday_category(year)
    if weekday_category:
        parts.append(weekday_category)
