...
## THE CYCLES LIVE IN NOTION NOW — READ THIS BEFORE PROPOSING ANY REPAIR

**<https://app.notion.com/p/3ae96556906781e98643c34a75bd8a86>** — *Ancestry cycles, every
one, with ancestor counts.* All 35, one section each, every member listed with its
**ancestor count**, descendant count, depth, and whether it reaches `Q1` Aster.

- **Source of truth is `wikibase/analysis/cycles_review.md`**, generated by
  `wiki-scripts/build_cycles_notion.py`. The Notion page is a copy. Regenerate the file and
  re-push the page after **any** change to `wikibase/items/`. See `docs/notion_sync.md`.
...
- **`ancestors` is the only column that ranks anything.** Cycle 5 has 29,135 descendants
  and 52 ancestors; cycle 18 has 34 descendants and 6,579 ancestors. Width and depth point
...
1. **Find the real defect in the Scipio loop — WITHOUT detaching the Scipiones from
   Aster.** This is cycle 3 on the Notion page. The `Q73893 → Q73794` cut was applied and then **reverted the same day**: it
