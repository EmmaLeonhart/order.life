...
- `calendar-lib/test_overview_preservation.py` is **dead** for the same reason — it needs a live-`wiki` pytest fixture (closed wiki), so it errors at setup (2 errors, fixture not found). `calendar-lib/test_page_generation.py` still **passes** (2/2, generation logic healthy). Don't fake the `wiki` fixture to make it green; it's blocked on a wiki existing.
