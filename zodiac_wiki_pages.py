...
BC_DATES_CSV = "gaian_bc_dates.csv"           # Summary year ranges
    ...
    try:
        with open(csv_path, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                ...
                # Parse BC dates back to Python date objects
                start_date_str = row['StartDate']
                end_date_str = row['EndDate']
                
                # Convert BC date strings back to date objects
                start_date = parse_bc_date_string(start_date_str)
                end_date = parse_bc_date_string(end_date_str)
                
                _bc_dates_cache[gaian_year] = {
                    'start_date': start_date,
                    'end_date': end_date,
                    'iso_year': int(row['ISOYear']),
                    'days_in_year': int(row['DaysInYear']),
                    'has_intercalary': row['HasIntercalary'].lower() == 'true',
                    'start_weekday': row['StartWeekday'],
                    'end_weekday': row['EndWeekday']
                }
